<div x-data="{ activeUser: false }" class="relative text-black hover:text-gray-700">
    ...
    <div x-show="activeUser" @click.away="activeUser = false" x-transition:enter="transition ease-out duration-200"
        ...
        class="absolute top-3/4 left-0 right-0 -translate-x-4 w-36 z-20 bg-white shadow-lg -md text-center mx-auto mt-6 overflow-hidden">
        @auth
        @if(Gate::check('isCustomer'))
        <a href="{{ route('customer.dashboard') }}"
            class="flex items-center px-4 py-2 text-basetransition-colors duration-200">
        ...
        <a href="{{ route('customer.orders') }}"
            class="flex items-center px-4 py-2 text-base hover:bg-gray-100 transition-colors duration-200">
            <svg class="w-5 h-5 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
        ...
        <form method="POST" action="{{ route('logout') }}">
            @csrf
            <button type="submit"
                class="w-full text-left flex items-center px-4 py-2 text-base hover:bg-gray-100 transition-colors duration-200">
                <svg class="w-5 h-5 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
