        ...
        <div class="bg-white  p-4 mb-3">
            <form method="GET" action="{{ route('admin.products.index') }}">
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-6 gap-4">
                    ...
                    <!-- Brand Filter -->
                    <div>
                        <label for="brand" class="block text-base font-medium text-gray-700 mb-1">Brand</label>
                    ...
                    <!-- Stock Filter -->
                    <div>
                        <label for="stock" class="block text-base font-medium text-gray-700 mb-1">Stock</label>
                        <select id="stock" name="stock"
                            class="mt-1 block w-full pl-3 pr-10 py-2 text-base border border-gray-300 focus:outline-none focus:ring-blue-500 focus:border-blue-500">
                            <option value="">All</option>
                            <option value="in_stock" {{ request('stock') == 'in_stock' ? 'selected' : '' }}>In Stock
                            </option>
                            <option value="out_of_stock" {{ request('stock') == 'out_of_stock' ? 'selected' : '' }}>Out
                                of
                                Stock</option>
                        </select>
                    </div>
                    ...
                    <!-- Rating Filter -->
                    <div>
                        <label for="rating" class="block text-base font-medium text-gray-700 mb-1">Rating</label>
                    ...
                    <!-- Price Filter -->
                    <div>
                        <label for="sort" class="block text-base font-medium text-gray-700 mb-1">Sort By Price</label>
                        <select id="sort" name="sort"
                            class="mt-1 block w-full pl-3 pr-10 py-2 text-base border border-gray-300 focus:outline-none focus:ring-blue-500 focus:border-blue-500 transition duration-150 ease-in-out">
                ...
                <div class="flex justify-end mt-4 space-x-3">
                    <a href="{{ route('admin.products.index') }}"
                        class="px-4 py-2 border border-gray-300 text-gray-700 hover: transition-colors">
                        Reset Filters
                    </a>
                    <button type="submit" class="px-4 py-2 bg-black  text-white  transition-colors">
                        Apply Filters
                    </button>
                </div>
            </form>
                ...
                <div class="p-4 border-b border-gray-200 flex justify-between items-center">
                    ...
                    <div>
                        <a href="{{ route('admin.products.create') }}"
                            class="inline-flex items-center px-4 py-2 border border-transparent shadow-sm text-base font-medium text-white bg-black   focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500">
                                ...
                                <tr class="hover: transition-colors">
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="flex items-center">
                                            ...
                                            <div class="ml-4">
                                                <div class="text-lg font-medium text-gray-900">{{ $product->title }}</div>
                                                <div class="text-base text-gray-500">
                                                    {{ Str::limit($product->short_des, 30) }}</div>
                                            </div>
                                        </div>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="text-base text-gray-900">
                                            @if ($product->discount > 0 && $product->discount_price < $product->price)
                                                ...
                                                <span class="text-base text-gray-500 line-through ml-1">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap" x-data="stockModal({{ $product->id }}, {{ $product->stock_quantity }})">
                                        ...
                                        {{ $product->stock_quantity <= $product->stock_alert ? 'bg-red-100 text-red-800' : 'bg-green-100 text-green-800' }}">
                                            ...
                                            <div class="bg-white p-6 w-80" @click.away="open = false">
                                                ...
                                                <form @submit.prevent="submit">
                                                    <input type="number" x-model="newQty"
                                                        class="w-full px-3 py-2 text-xl font-semibold border border-gray-300 focus:outline-none focus:ring focus:border-black"
                                                        min="0" required>

                                                    <div class="mt-4 flex justify-end space-x-2">
                                                        <button type="button" @click="open = false"
                                                            class="px-4 py-2 bg-gray-200 text-gray-700 hover:bg-gray-300">
                                                            Cancel
                                                        </button>
                                                        <button type="submit" class="px-10 py-2 bg-black text-white"
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="flex items-center">
                                            ...
                                            <span
                                                class="ml-1 text-base text-gray-600">{{ number_format($product->avg_rating, 1) ?? 'N/A' }}</span>
                                        </div>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <select
                                            class="px-2 py-1 text-base font-semibold rounded-full focus:outline-none focus:ring 
                ...
                : ($product->status == 'inactive'
                    ...
                    : 'bg-gray-100 text-gray-800') }}"
                                            ...
                                            <option value="discontinued"
                                                {{ $product->status == 'discontinued' ? 'selected' : '' }}>Discontinued
                                            </option>
                                        </select>
                                    </td>
