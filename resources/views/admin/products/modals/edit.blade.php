...
<div class="container mx-auto p-4">
    <div class="bg-white shadow-md p-6">
        ...
        <form id="productForm" method="POST" action="{{ route('admin.products.update', $product->id) }}"
                ...
                <div class="grid grid-cols-1 gap-y-6 gap-x-4 sm:grid-cols-12">
                    ...
                    <div class="sm:col-span-4">
                        <label for="title" class="block text-sm font-medium text-gray-700">Product Title <span
                                class="text-red-500">*</span></label>
                        <div class="mt-1">
                            <input type="text" id="title" value="{{ old('title', $product->title) }}" name="title"
                    ...
                    <div class="sm:col-span-6">
                        ...
                        <div class="mt-1">
                            <textarea id="short_des" name="short_description" rows="9" maxlength="500"
                                ...
                                class="block w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">{{ old('short_description', $product->short_description) }}</textarea>
                        </div>
                    </div>
                    ...
                    <div class="sm:col-span-6">
                        ...
                        <div id="image-upload-container" class="relative group cursor-pointer">
                            ...
                            <div id="upload-box"
                                class="flex flex-col items-center justify-center h-48 border-2 border-dashed border-gray-300 bg-gray-50 hover:bg-gray-100 transition-colors {{ $product->images->where('is_primary', true)->first() ? 'hidden' : '' }}">
                            ...
                            <div id="image-preview-container"
                                class="relative h-48 overflow-hidden bg-gray-100 {{ $product->images->where('is_primary', true)->first() ? '' : 'hidden' }}">
                                @if($primaryImage = $product->images->where('is_primary', true)->first())
                                <img id="image-preview" src="{{ Storage::url($primaryImage->image_path) }}"
            ...
            <div class="pt-8">
                <div class="mb-6">
                    <h3 class="text-xl font-semibold leading-6 text-gray-900">Additional Images</h3>
                    <p class="mt-1 text-sm text-gray-500">Upload additional product images (max 5 total).</p>
                </div>

                <!-- Existing additional images -->
                @if($product->images->where('is_primary', false)->count() > 0)
                <div class="mb-6">
                    <h4 class="text-md font-medium text-gray-700 mb-3">Current Additional Images</h4>
                    <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-4" id="existing-additional-images">
                        @foreach($product->images->where('is_primary', false) as $image)
                        <div class="relative group border bg-gray-50 existing-image-container">
                            <img src="{{ Storage::url($image->image_path) }}" alt="Product image"
                                class="w-full h-32 object-cover rounded">
                            <input type="hidden" name="existing_images[]" value="{{ $image->id }}">
                            <button type="button"
                                class="absolute top-1 right-1 bg-red-600 text-white rounded-full p-1 remove-existing-image">
                ...
                <div class="mb-6">
                    <h4 class="text-md font-medium text-gray-700 mb-3">Add New Images</h4>
                    <p class="text-xs text-gray-500 mb-3">Select up to {{ 5 - $product->images->where('is_primary',
                        false)->count() }} additional images (max 500KB each)</p>
...
<script>
    ...
    const checkSkuAvailability = (sku) => {
        ...
        fetch('{{ route("admin.products.checkSku") }}', {
            ...
            body: JSON.stringify({ 
                sku: sku,
                product_id: {{ $product->id }} // Include product ID for edit validation
            })
        })
...
<script>
    ...
    // Function to calculate max allowed images
    function calculateMaxImages() {
        const existingImagesCount = document.querySelectorAll('.existing-image-container').length;
        return 5 - existingImagesCount;
    }

    // Function to update max images message
    function updateMaxImagesMessage() {
        const maxImages = calculateMaxImages();
        maxImagesMessage.textContent = `Select up to ${maxImages} additional images (max 500KB each)`;
        return maxImages;
    }

    // Remove existing image from frontend only
    document.querySelectorAll('.remove-existing-image').forEach(button => {
        button.addEventListener('click', function() {
            ...
            // Update the max number of additional images message
            updateMaxImagesMessage();
        });
    });
    ...
    function handleFiles(files) {
        ...
        // Calculate max allowed images dynamically
        const MAX_IMAGES = calculateMaxImages();

        // Check if we're exceeding the maximum allowed images
        if (selectedFiles.length + files.length > MAX_IMAGES) {
            errorElement.textContent = `You can only upload up to ${MAX_IMAGES} additional images.`;
    ...
    function createPreview(file, index) {
        ...
        reader.onload = function(e) {
            ...
            preview.className = 'relative group border bg-gray-50';
            preview.innerHTML = `
                <img src="${e.target.result}" alt="Preview" class="w-full h-32 object-cover rounded">
                <button type="button" data-index="${index}" class="remove-image-btn absolute top-1 right-1 bg-red-600 text-white rounded-full p-1">
