                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            @foreach ($categories as $category)
                                <tr>
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="text-lg font-medium">{{ $category->category_name }}</div>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="text-lg">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <select
                                            class="px-4 py-1 text-sm font-semibold rounded-full focus:outline-none focus:ring 
        {{ $category->status == 'active' ? 'bg-gray-200 text-black' : 'bg-gray-100 text-black' }}"
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                        <div class="flex space-x-2">
                                            <button
                                                @click="openEditModal({{ $category->id }}, '{{ addslashes($category->category_name) }}', '{{ asset('storage/' . $category->categoryImg) }}', {{ $category->parent_id ?? 'null' }}, '{{ $category->status }}')"
            ...
            Alpine.data('categoryManagement', () => ({
                ...
                async submitEditForm() {
                    ...
                    try {
                        ...
                        if (this.editForm.newLogo) formData.append('categoryImg', this.editForm
                            .newLogo);
