                    ...
                    <div class="relative flex gap-4 items-start py-6 border-t-2 border-gray-200 cart-item"
                        ...
                        <div class="flex-1 min-w-0 font-semibold">
                            ...
                            <div class="mb-4">
                                ...
                                $selectedColorId = $item->variant ? $item->variant->color_id : null;
                                @endphp
                            ...
                            <div class="mb-4">
                                ...
                                @if($item->product->variants && $item->product->variants->count() > 0)
                                @php
                                $selectedSizeId = $item->variant ? $item->variant->size_id : null;
                                $selectedColorId = $item->variant ? $item->variant->color_id : null;

                                // Get available sizes based on selected color
                                $availableSizes = $selectedColorId ?
                                $item->product->variants->where('color_id',
                                $selectedColorId)->pluck('size')->unique('id') :
                                collect();
                                @endphp
                            ...
                            <div class="mb-4">
                                <div class="flex flex-wrap items-center gap-2">
                                    ...
                                    <p class="text-base text-gray-800 item-price" id="price-{{ $item->id }}">
    ...
    function initCart() {
        ...
        cartItems.forEach(item => {
            ...
            cartState.items.set(itemId, {
                ...
                colorId: item.querySelector('.color-option.selected')?.dataset.colorId || null,
                sizeId: item.querySelector('.size-option.selected')?.dataset.sizeId || null
            });

            checkbox.addEventListener('change', () => updateItemSelection(itemId, checkbox.checked));
            qtySelect.addEventListener('change', () => updateItemQty(itemId, parseInt(qtySelect.value)));
    ...
    function selectColor(itemId, productId, colorId, colorName) {
        ...
        .then(sizes => {
            ...
            if (sizes.length > 0) {
                ...
                sizesContainer.innerHTML = '<p class="text-sm text-gray-500">No sizes available</p>';
        ...
        .catch(error => {
            ...
            itemElement.querySelector('.size-options').innerHTML = '<p class="text-sm text-red-500">Error loading sizes</p>';
        })
        .finally(() => {
